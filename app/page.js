...
import { compact, nf, agoRu, dateRu, proxy } from '@/lib/format';
...
export default async function Dashboard({ searchParams }) {
  ...
  const [owner] = uid === Number(me.id) ? [me] : await sql`select display_name, handle from users where id = ${uid}`;

  const [kpi, series, top] = await Promise.all([getKpi(uid), getSeries(uid, 30), getTop(uid, 5)]);
  ...
  const er = Number(kpi.views) > 0
    ? (((Number(kpi.likes) + Number(kpi.comments)) / Number(kpi.views)) * 100).toFixed(2)
    : '0.00';
  ...
  return (
    <>
      ...
      <section className="mb-6 grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <Stat label="Просмотров"      value={kpi.views}     emoji="👀" tone="grape" sub={`в среднем ${compact(kpi.avg_views)} на рилс`} delay={60} />
        <Stat label="Лайков"          value={kpi.likes}     emoji="💗" tone="rose"  sub={`${compact(kpi.comments)} комментариев`} delay={120} />
        <Stat label="Вовлечённость"   value={er}            emoji="✨" tone="mint"  sub="(лайки + комменты) / просмотры" delay={180} />
      </section>
            ...
            : <ol className="flex flex-col gap-3">
                {top.map((r, i) => (
                  <li key={r.id} className="flex items-center gap-3">
                    <span className="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-blush/70 font-display text-sm">{i + 1}</span>
      ...
      <Link href="/reels" className="card rise flex items-center justify-between gap-4 p-6 transition hover:-translate-y-0.5">
        <div>
          <div className="font-display text-2xl">Добавить рилс</div>
          <div className="text-sm text-mute">Вставь ссылку из Instagram — обложка, дата и просмотры подтянутся сами</div>
        </div>
